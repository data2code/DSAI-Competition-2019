...
def unix(exe, l_print=True, l_error=True, l_block=True, error_to_stdout=True, sleep_time=0.5):
    ...
    if l_block:
        ...
        p = subprocess.Popen(exe, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True, bufsize=1, close_fds=ON_POSIX)
    else:
        return subprocess.Popen(exe, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
...
def html3(df, portrait=True, theme="blue"):
    ...
    return html(df, colors=colors, tag_table={'border':'1'}, portrait=portrait)
